...
no.of.levels <- 7 # How many times a ball chooses to fall left or right
...
temp_df = data.frame(a=rep(0, iterations))
col_nums <- paste0("circle",2:(no.of.circles + 1))
temp_df[col_nums] <- rep(0,iterations)
temp_df <- subset (temp_df, select = -a)
iteration_circles <- cbind(iteration_circles,temp_df)
...
viz.final <- animint(viz,viz2)


info <- animint2HTML(viz.final)

test_that("All triangles present", {
  line.list <-
    getNodeSet(info$html, '//svg[@id="plot_viz"]//g[@class="geom1_polygon_viz"]//path')
  trigs.count <- length(line.list)
  expect_equal(trigs.count, (no.of.levels+1)*(no.of.levels+2)/2)
})
